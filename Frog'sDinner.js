...
// Solution
const frogContest = (n) => {
  const chrisFlies = sum(n);
  const tomFlies = sum(Math.floor(chrisFlies / 2));
  const manyFlies = sum(chrisFlies + tomFlies);

  return `Chris ate ${chrisFlies} flies, Tom ate ${tomFlies} flies and Cat ate ${manyFlies} flies`;
};
